					...
					for i = 1, total do
						...
						if connected and level >= guild.threshold then
							...
							if self.altdown then
								...
								GameTooltip:AddDoubleLine(format("|cff%02x%02x%02x%d|r %s%s%s",levelc.r*255,levelc.g*255,levelc.b*255,level,name,sflag,grouped),zone,classc.r,classc.g,classc.b,zone_r,zone_g,zone_b)
							end
						end
					end
					GameTooltip:AddLine' '
...
if stats.enabled then
	local function tags(sub)
		...
		elseif sub == "mastery" then 
		    string = GetCombatRatingBonus(26)
