...
local CreateLootSlot = function(self, id)
	...
	slot:SetPoint("TOPLEFT", 3, -20 - (id - 1) * (slotSize + 3))
...
local OnEvent = function(self, event, ...)
	if event == "ADDON_LOADED" then
		...
		if name == "alLootFrames" then
			...
			local Frameshadow = CreateFrame("Frame", nil, self)
			Frameshadow:SetBackdrop{
				edgeFile = "Interface\\AddOns\\Litepanels\\media\\glowTex",
				tile = false, tileSize = 16, edgeSize = 5,
				insets = {left = -5, right = 5, top = 5, bottom = -5},
			}
			Frameshadow:SetBackdropBorderColor(0,0,0,1)
			Frameshadow:SetPoint("TOPLEFT", -5, 5)
			Frameshadow:SetPoint("BOTTOMRIGHT", 5, -5)
