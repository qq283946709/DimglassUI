  ...
  if barcfg.uselayout2x6 then
    ...
    bar:SetWidth(barcfg.buttonsize*18+barcfg.buttonspacing*17)
    bar:SetHeight(barcfg.buttonsize*2)
  end
  if barcfg.uselayout2x6 then
  ...
  for i=7, 12 do
    ...
    button:ClearAllPoints()
    if i == 7 then
      button:SetPoint("BOTTOMRIGHT", bar, 0,0)
    else
      local previous = _G["MultiBarBottomRightButton"..i-1]      
      if  i == 10 then
        ...
        button:SetPoint("RIGHT", previous, "LEFT", -barcfg.buttonspacing, 0)
      end
