...
local shadows = {
	edgeFile = "Interface\\AddOns\\NugRunning\\glowTex", 
	edgeSize = 5,
	tile = false,
	insets = { left = 0, right = 0, top = 0, bottom = 0 }
}

local CreateBorder = function(f,x)
	...
	border:SetBackdropBorderColor(65/255, 74/255, 79/255)
end

local CreateShadow = function(f,x)
...
local TimerOnSettingsChanged = function (self)
    ...
    self.bar:SetWidth(width-height-5)
    self.bar:SetHeight(height)
    self.timeText:SetFont(GameTooltipText:GetFont(),height*.5*fontscale,"OUTLINE")
    self.spellText:SetFont(GameTooltipText:GetFont(),height*.5*fontscale,"OUTLINE")
    self.spellText:SetWidth(self.bar:GetWidth()*0.8)
    self.stacktext:SetFont(GameTooltipText:GetFont(),height*.5*fontscale,"OUTLINE")
end
NugRunning.BarFrame = function(f)
    ...
    f.stacktext:SetFont(GameTooltipText:GetFont(),height*.5*fontscale,"OUTLINE")
    ...
    f.bar:SetWidth(width - height - 5)
    f.bar:SetPoint("TOPRIGHT",f,"TOPRIGHT",0,0)
	CreateBorder(f.bar,1)
	CreateShadow(f.bar,5)
    ...
    f.timeText:SetFont(GameTooltipText:GetFont(),height*.5*fontscale,"OUTLINE")
    f.timeText:SetJustifyH("RIGHT")
    f.timeText:SetVertexColor(1,1,1)
    f.timeText:SetPoint("RIGHT", f.bar, "RIGHT",-6,0)
    
    f.spellText = f.bar:CreateFontString();
    f.spellText:SetFont(GameTooltipText:GetFont(),height*.5*fontscale,"OUTLINE")
...
--~         f.timeText:SetFont(GameTooltipText:GetFont(),height*.7) --font size here
