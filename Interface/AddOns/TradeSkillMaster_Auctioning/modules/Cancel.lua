...
function Cancel:CancelMatch(match)
	...
	for i=GetNumAuctionItems("owner"), 1, -1 do
		local name, quantity, bid, buyout, wasSold
		if select(4, GetBuildInfo()) >= 40300 then -- fix for added paramter in 4.3
			name, _, quantity, _, _, _, _, bid, _, buyout, _, _, _, wasSold = GetAuctionItemInfo("owner", i)
		else
			name, _, quantity, _, _, _, bid, _, buyout, _, _, _, wasSold = GetAuctionItemInfo("owner", i)
		end
...
function Cancel:CancelAll(filter)
	...
	if tonumber(parsedArg) then
		...
		if parsedArg ~= 12 and parsedArg ~= 2 and parsedArg ~= 0.5 then
			TSMAuc:Print(string.format(L["Invalid time entered, should either be 12, 2, or .5 you entered \"%s\""], parsedArg))
			return Cancel:StopCanceling()
		end
		duration = (parsedArg == 12 and 3) or (parsedArg == 2 and 2) or 1--3 = <12 hours, 2 = <2 hours, 1 = <30 min
	elseif TSMAuc.db.profile.groups[parsedArg] then
	...
	if duration then
		TSMAuc.Log:AddMessage(string.format(L["Mass cancelling posted items with less than %d hours left"], (duration == 3 and 12) or (duration == 2 and 2) or 0.5), "masscancel")
	elseif group then
...
function Cancel:StartScan()
	...
	for i=GetNumAuctionItems("owner"), 1, -1 do
		local isNotSold
		if select(4, GetBuildInfo()) >= 40300 then -- fix for added paramter in 4.3
			isNotSold = select(14, GetAuctionItemInfo("owner", i)) == 0
		else
			isNotSold = select(13, GetAuctionItemInfo("owner", i)) == 0
		end
...
function Cancel:QueueItemToCancel(itemID, noLog)
	...
	for _, index in pairs(tempList) do
		local name, quantity, bid, buyout, activeBid, highBidder, wasSold
		if select(4, GetBuildInfo()) >= 40300 then -- fix for added paramter in 4.3
			name, _, quantity, _, _, _, _, bid, _, buyout, activeBid, highBidder, _, wasSold = GetAuctionItemInfo("owner", index)
		else
			name, _, quantity, _, _, _, bid, _, buyout, activeBid, highBidder, _, wasSold = GetAuctionItemInfo("owner", index)
		end
		...
		if wasSold == 0 and lowestOwner then
			...
			elseif resetMethod ~= "none" and lowestBuyout <= TSMAuc.Config:GetConfigValue(itemID, "threshold") then
				...
				if resetBuyout and abs(resetBuyout - buyoutPerItem) < quantity then -- use the quantity as a buffer to account for rounding
					if not noLog then
						msgTable.scanData.action="Skip"
						TSMAuc.Log:AddMessage(msgTable, name.."notcancel")
					end
					...
					totalToCancel = totalToCancel + 1
					tinsert(cancelQueue, {itemID=itemID, stackSize=quantity, buyout=buyout, bid=bid, index=index})
				end
			-- It is supposed to be cancelled!
				...
				if isPlayer and priceDifference and priceDifference >= priceThreshold then
					...
					if floor(lowestBuyout) == floor(buyoutPerItem) and undercut ~= 0 then
						if not noLog then
							msgTable.scanData.action="Cancel"
							TSMAuc.Log:AddMessage(msgTable, name.."cancel")
						end
				...
				elseif (not isPlayer and not isWhitelist) or
					isWhitelist and (buyoutPerItem > lowestBuyout or (buyoutPerItem == lowestBuyout and lowestBid < bidPerItem)) or
					TSMAuc.db.global.smartCancel and TSMAuc.Scan:IsPlayerOnly(itemID) and buyoutPerItem < fallback then
...
function Cancel:CancelItem()
	...
	for i=GetNumAuctionItems("owner"), 1, -1 do
		local quantity, bid, buyout, activeBid
		if select(4, GetBuildInfo()) >= 40300 then -- fix for added paramter in 4.3
			_, _, quantity, _, _, _, _, bid, _, buyout, activeBid = GetAuctionItemInfo("owner", i)
		else
			_, _, quantity, _, _, _, bid, _, buyout, activeBid = GetAuctionItemInfo("owner", i)
		end
		local itemID = TSMAuc:GetItemString(GetAuctionItemLink("owner", i))
		if itemID == currentItem.itemID and abs((buyout or 0) - (currentItem.buyout or 0)) < quantity and abs((bid or 0) - (currentItem.bid or 0)) < quantity and (not TSMAuc.db.global.cancelWithBid and activeBid == 0 or TSMAuc.db.global.cancelWithBid) then
	...
	if not cancelError then -- didn't get "item not found" for any cancels so we are done
		...
		for i=GetNumAuctionItems("owner"), 1, -1 do
			...
			elseif status.type == "all" then
				...
				local timeLeft = GetAuctionItemTimeLeft("owner", i)
				if wasSold == 0 and (not group or TSMAuc.itemReverseLookup[itemID] == group) and (not duration or timeLeft <= duration) and (not price or buyout <= price) then
					totalToCancel = totalToCancel + 1
					tinsert(cancelQueue, {itemID=itemID, stackSize=quantity, buyout=buyout, bid=bid, index=i})
				end
