...
local shadows = {
	edgeFile = "Interface\\AddOns\\Filger\\glowTex", 
	edgeSize = 5,
	tile = false,
	insets = { left = 0, right = 0, top = 0, bottom = 0 }
}
		...
		if (not bar) then
			...
			if (self.Mode == "ICON") then
				...
				if (bar.count) then
					...
					bar.count:SetFont(GameTooltipText:GetFont(), 14, "OUTLINE")
				...
				if (bar.statusbar) then
					...
					CreateBorder(bar.statusbar, 1)
					CreateShadow(bar.statusbar, 6)
				...
				if (bar.time)then
					...
					bar.time:SetFont(GameTooltipText:GetFont(), 14, "OUTLINE")
				...
				if (bar.spellname)then
					...
					bar.spellname:SetFont(GameTooltipText:GetFont(), 14, "OUTLINE")
